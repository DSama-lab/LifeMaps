...
- **Google-Maps-style chrome.** A left navigation drawer (Plan / Your data / Share / Customization / Account), a round account avatar with a dropdown, a top search box with live place/item suggestions that fly the camera to the target, status chips with counters that filter both the map pins and the 2D graph, and a Google-style apps menu (nine-dot button next to the avatar) with eight inline-SVG tiles linking to the owner's services (the travel blog placeholder, Música na Pauta, the owner's store, Gmail, Drive, YouTube, ChatGPT, Canva).
