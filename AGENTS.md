...
- **Modos de arrasto touchpad (configurável, 2026-09-13)**: `#mapPanMode` (select no gear ⚙️) + `mapPanMode` (`mv_pan_mode`, default `diag`). `mmTrackpadPan` despacha por modo: **`diag`** → `mmDiagDelta` (sempre diagonal); **`diagonly`** → usa `mmAxisKind(dx,dy)` (0=nulo, 1=H-dominante, 2=V-dominante, 3=diagonal); se `k<3` ignora o gesto (só diagonais arrastam; V/H ficam travados de propósito); **`native`** → `[deltaX,deltaY]` puro (comportamento do navegador). `setGestMode(mode)` persiste e dá toast; `toggleMapGear` reflete o valor salvo.
