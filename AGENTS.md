...
| `test-map.html` | Harness do Mapa Premium (17/17): pins sem duplicação + `project()`, card compacto→full + anel `.sel`, touch-guard, troca de estilo (glyphs CORS-safe, `traj-line` converge), diagonalização do touchpad, busca, criação/limpeza de item. Contagem de pins é **dinâmica** (`pinCount` do boot). **Atenção**: usa `dispatchEvent` — não valida hit-test/clique físico (isso é coberto pela validação mobile Playwright). |
